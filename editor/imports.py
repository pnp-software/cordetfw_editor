...
def import_project_tables(request, imp_dir):
    ...
    for spec_item in spec_items:
        try:
            new_spec_item = SpecItem(cat = spec_item['cat'].strip(),
                                     name = spec_item['name'].strip(),
                                     domain = spec_item['domain'].strip(),
                                     project = new_project,
                                     application = application_old_id_2_new[spec_item['application']],
                                     title = spec_item['title'].strip(),
                                     desc = spec_item['desc'].strip(),
                                     owner = user_name_2_user[spec_item['owner']],
                                     val_set = val_set_old_id_2_new[spec_item['val_set']],
                                     status = spec_item['status'].strip(),
                                     updated_at = make_aware(datetime.datetime.strptime(spec_item['updated_at'], "%Y-%m-%d %H:%M:%S")),
                                     rationale = spec_item['rationale'].strip(),
                                     implementation = spec_item['implementation'].strip(),
                                     remarks = spec_item['remarks'].strip(),
                                     p_kind = spec_item['p_kind'].strip(),
                                     s_kind = spec_item['s_kind'].strip(),
                                     value = spec_item['value'].strip(),
                                     t1 = spec_item['t1'].strip(),
                                     t2 = spec_item['t2'].strip(),
                                     t3 = spec_item['t3'].strip(),
                                     t4 = spec_item['t4'].strip(),
                                     t5 = spec_item['t5'].strip(),
                                     n1 = spec_item['n1'],
                                     n2 = spec_item['n2'],
                                     n3 = spec_item['n3'],
                                     change_log = spec_item['change_log'].strip())
            new_spec_item.save()
